...
from verl.utils.dataset import SFTDataset, SFTMTDataset
...
class FSDPSFTMTTrainer(object):
    ...
    def _build_dataloader(self):
        config = self.config
        # build dataset
        self.train_dataset = SFTMTDataset(parquet_files=config.data.train_files,
                                        tokenizer=self.tokenizer,
                                        prompt_key=config.data.prompt_key,
                                        prompt_dict_keys=config.data.get('prompt_dict_keys', None),
                                        max_length=config.data.max_length,
                                        truncation=config.data.truncation)
        self.val_dataset = SFTMTDataset(parquet_files=config.data.val_files,
                                      tokenizer=self.tokenizer,
                                      prompt_key=config.data.prompt_key,
                                      prompt_dict_keys=config.data.get('prompt_dict_keys', None),
                                      max_length=config.data.max_length,
...
from verl.trainer.fsdp_sft_mt_trainer import FSDPSFTMTTrainer
import hydra
...
def main(config):
    ...
    trainer = FSDPSFTMTTrainer(config=config, device_mesh=device_mesh)
    trainer.fit()
