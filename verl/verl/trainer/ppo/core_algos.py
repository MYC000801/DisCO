...
def compute_policy_loss_discob_Lratio(old_log_prob, log_prob, advantages, eos_mask, uid, seq_level_rewards, delta, beta, kl_type='low_var_kl'):
    ...
    global_scores= (ratio * global_eos_mask).sum(dim=1) / global_eos_mask.sum(dim=1)
    ...
    if valid_mask.sum()>0:
        ...
        neg_mean = (grouped_scores* neg_mask).sum(dim=1, keepdim=True)/neg_mask.sum(dim=1, keepdim=True)
        # if torch.distributed.get_rank()==0:
        #     print('grouped_scores', grouped_scores)

        pg_losses = ((grouped_scores - neg_mean)*pos_mask
...
def compute_policy_loss_discob_logL(old_log_prob, log_prob, advantages, eos_mask, uid, seq_level_rewards, delta, beta,  kl_type='low_var_kl'):
    """
...
def compute_policy_loss_disco_Lratio(old_log_prob, log_prob, advantages, eos_mask, uid, seq_level_rewards, delta, beta, tau, kl_type='low_var_kl'):
    """
