...
class MazeInteraction(BaseInteraction):
    ...
    async def generate_response(self, instance_id: str, messages: List[Dict[str, Any]], **kwargs) -> Tuple[bool, str, float, dict]:
        ...
        should_terminate_sequence, response = await self.calculate_feedback(instance_id)
        reward = 0.0
    ...
    async def calculate_feedback(self, instance_id: str, **kwargs):
        ...
        if model_move in valid_moves:
            ...
            if x == goal[0] + 1 and y == goal[1] + 1:
                print(f"Reached goal at {pos}")
                response = "Congratulations! You have reached the goal!"
                should_terminate_sequence = True
            else:
                # 生成新的观察
                maze_grid = self._instance_dict[instance_id]["maze"]
                observation_chat = maze_mt.get_observation_chat(pos, maze_grid)
                # 提取观察内容
                if observation_chat and len(observation_chat) > 0:
                    response = observation_chat[0]["content"]
                else:
                    response = "Unable to generate observation."
                should_terminate_sequence = False
        else:
            response = f"Your response is incorrect! Valid moves are: {', '.join(valid_moves)}. Please try again."
            should_terminate_sequence = True

        return should_terminate_sequence, response
